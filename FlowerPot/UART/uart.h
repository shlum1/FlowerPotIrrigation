 ...
 *  @param  baudrate baudrate in bps, e.g. 1200, 2400, 9600 ... 57600UL; (special treatment for 115200! @ 20MHz)     
 */
...
//#define UART_BAUD_SELECT(baudRate,xtalCpu) (25)   
//#define UART_BAUD_SELECT(baudRate,xtalCpu) (   ((xtalCpu) / ((baudRate) * 16)) - 1 )  //
#define UART_BAUD_SELECT(baudRate,xtalCpu) ((unsigned int) ((1.0*(xtalCpu) / ((baudRate) * 16.0)) - 0.5) )  //-1 +0.5 for better rounding
...
#define UART_TX_BUFFER_SIZE 256
  ...
  #define __uart_write(...)  uart_write(__VA_ARGS__)
  #define __uart_puts_P(s) uart_puts_P(s)
  #define __uart_puti(i) uart_puti(i)
#else
  #define __uart_write(...)
  #define __uart_puts_P(s)
  #define __uart_puti(i)
#endif
...
/**
   ...
   @return  none
*/
//void uart_init(unsigned long baud); // baudrate calculated internally
...
//#define uart_write(...) 
  ...
  extern void _uart_write_p(const char *progmem_s, ... );
  #define uart_write_P(__s, ...)       _uart_write_p(P(__s), __VA_ARGS__)
